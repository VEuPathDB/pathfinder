...
class TestResolveGeneIds:
    ...
    @patch("veupath_chatbot.services.gene_lookup.wdk.get_wdk_client")
    async def test_empty_ids_returns_empty(self, mock_get_client: MagicMock) -> None:
        result = await resolve_gene_ids("plasmodb", [])
        assert result == {"records": [], "totalCount": 0}
        mock_get_client.assert_not_called()

    @patch("veupath_chatbot.services.gene_lookup.wdk.get_wdk_client")
    async def test_successful_resolution(self, mock_get_client: MagicMock) -> None:
        mock_client = AsyncMock()
        # First call: create dataset
        mock_client.post = AsyncMock(
            side_effect=[
                {"id": 12345},  # dataset creation response
                {  # standard reporter response
                    "meta": {"totalCount": 1},
                    "records": [
                        {
                            "attributes": {
                                ...
                                "primary_key": "PF3D7_0100100",
                                "gene_name": "CSP",
                                "gene_product": "circumsporozoite protein",
                                "organism": "Plasmodium falciparum 3D7",
                                "gene_type": "protein coding",
                                "gene_location_text": "chr3:100-500(+)",
                                "gene_previous_ids": "",
                            },
                        }
                    ],
        ...
        result = await resolve_gene_ids("plasmodb", ["PF3D7_0100100"])
        records = result["records"]
        assert isinstance(records, list)
        assert len(records) == 1
        first = records[0]
        assert isinstance(first, dict)
        assert first["geneId"] == "PF3D7_0100100"
        assert first["geneName"] == "CSP"
        assert result["totalCount"] == 1

    @patch("veupath_chatbot.services.gene_lookup.wdk.get_wdk_client")
    async def test_dataset_creation_returns_non_dict(
        self, mock_get_client: MagicMock
    ) -> None:
        mock_client = AsyncMock()
        mock_client.post = AsyncMock(return_value="not a dict")
        ...
        result = await resolve_gene_ids("plasmodb", ["GENE_A"])
        assert result["error"] == "Failed to create dataset for ID lookup."
        assert result["records"] == []

    @patch("veupath_chatbot.services.gene_lookup.wdk.get_wdk_client")
    async def test_dataset_creation_returns_no_id(
        self, mock_get_client: MagicMock
    ) -> None:
        mock_client = AsyncMock()
        ...
        result = await resolve_gene_ids("plasmodb", ["GENE_A"])
        assert result["error"] == "Dataset creation returned no ID."

    @patch("veupath_chatbot.services.gene_lookup.wdk.get_wdk_client")
    async def test_wdk_exception_returns_error(
        self, mock_get_client: MagicMock
    ) -> None:
        mock_client = AsyncMock()
    ...
    @patch("veupath_chatbot.services.gene_lookup.wdk.get_wdk_client")
    async def test_answer_non_dict_returns_empty(
        self, mock_get_client: MagicMock
    ) -> None:
        mock_client = AsyncMock()
        mock_client.post = AsyncMock(
    ...
    @patch("veupath_chatbot.services.gene_lookup.wdk.get_wdk_client")
    async def test_answer_non_list_records_returns_empty(
        self, mock_get_client: MagicMock
    ) -> None:
        mock_client = AsyncMock()
        mock_client.post = AsyncMock(
    ...
    @patch("veupath_chatbot.services.gene_lookup.wdk.get_wdk_client")
    async def test_custom_search_name_and_param(
        self, mock_get_client: MagicMock
    ) -> None:
        mock_client = AsyncMock()
        mock_client.post = AsyncMock(
        ...
        # Check the second call (reporter) used custom search name
        second_call = mock_client.post.call_args_list[1]
        assert "CustomSearch" in second_call[0][0]

    @patch("veupath_chatbot.services.gene_lookup.wdk.get_wdk_client")
    async def test_meta_fallback_when_not_dict(
        self, mock_get_client: MagicMock
    ) -> None:
        mock_client = AsyncMock()
        mock_client.post = AsyncMock(
