...
from veupath_chatbot.integrations.veupathdb.discovery import _filter_searches


def _search(url_segment: str, param_names: list[str]) -> dict:
    return {"urlSegment": url_segment, "paramNames": param_names}
...
def test_excludes_search_whose_param_matches_prefix() -> None:
    ...
    assert len(result) == 1
    assert result[0]["urlSegment"] == "GenesByText"
...
def test_excludes_search_when_only_one_param_matches() -> None:
    ...
    result = _filter_searches(searches, ["eda_"])
    assert len(result) == 1
    assert result[0]["urlSegment"] == "CleanSearch"
