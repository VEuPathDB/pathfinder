...
class TestModelCatalog:
    ...
    def test_all_models_unique(self):
        models = [e.model for e in MODEL_CATALOG]
    ...
    def test_all_providers_are_valid(self):
        valid_providers: set[ModelProvider] = {"openai", "anthropic", "google"}
        for entry in MODEL_CATALOG:
    ...
    def test_has_openai_entries(self):
        openai_entries = [e for e in MODEL_CATALOG if e.provider == "openai"]
        assert len(openai_entries) >= 1

    def test_has_anthropic_entries(self):
        anthropic_entries = [e for e in MODEL_CATALOG if e.provider == "anthropic"]
        assert len(anthropic_entries) >= 1

    def test_has_google_entries(self):
        google_entries = [e for e in MODEL_CATALOG if e.provider == "google"]
...
class TestGetModelEntry:
    def test_existing_model(self):
        entry = get_model_entry("openai/gpt-4o")
