...
export function StepAnalysisPanel({ entityRef }: StepAnalysisPanelProps) {
  ...
  useEffect(() => {
    ...
    void loadTypes(async () => {
      const { analysisTypes: types } = await getAnalysisTypes(entityRef);
  ...
  const handleRun = useCallback(async () => {
    ...
    await executeAnalysis(async () => {
      const result = await runAnalysis(entityRef, selectedType.name, paramValues);
      setResults(result);
    });
  }, [entityRef, selectedType, paramValues, executeAnalysis]);
    ...
    <div className="space-y-4">
      ...
      {selectedType && (
        <div className="space-y-3 rounded-lg border border-border bg-card p-4">
          ...
          {results &&
            (results._resultType === "enrichment" &&
            Array.isArray(results.enrichmentResults) ? (
              <EnrichmentSection
                results={results.enrichmentResults as EnrichmentResult[]}
              />
            ) : (
              <AnalysisResultView data={results} />
            ))}
        </div>
      )}
    </div>
