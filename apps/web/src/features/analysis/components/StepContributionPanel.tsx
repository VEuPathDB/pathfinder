...
import { flattenLeaves, planStepChildren } from "@/features/analysis/utils/treeUtils";
...
export function StepContributionPanel({
  ...
  const id = experimentId ?? geneSetId!;
  const basePath = geneSetId ? `/api/v1/gene-sets/${id}` : `/api/v1/experiments/${id}`;
  ...
  const fetchContributions = useCallback(async () => {
    ...
    await run(async () => {
      const data = await requestJson<{ contributions: StepContribution[] }>(
        `${basePath}/step-contributions`,
        { method: "POST", body: { stepTree } },
      );
      setContributions(data.contributions);
    });
  }, [basePath, stepTree, leafSteps.length, run]);
