...
describe("parseSSEChunk", () => {
  ...
  it("parses a single complete SSE frame with default event type", () => {
    ...
    expect(events).toEqual([{ type: "message", data: "hello", id: undefined }]);
    expect(rest).toBe("");
  });

  it("parses a frame with an explicit event type", () => {
    const { events, rest } = parseSSEChunk("event: status\ndata: ok\n\n");
    expect(events).toEqual([{ type: "status", data: "ok", id: undefined }]);
    expect(rest).toBe("");
  });
  ...
  it("handles \\r\\n line endings", () => {
    ...
    expect(events).toEqual([{ type: "ev", data: "d", id: undefined }]);
  });
